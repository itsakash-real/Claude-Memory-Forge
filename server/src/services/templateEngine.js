...
export function buildClaudeMd(refinedSections, answers) {
  ...
  let routingTriggers = refinedSections.routingTriggers || '';

  if (answers?.preferences?.wantsDashboard) {
...
export function buildFileMap(session) {
  ...
  // 19. Dashboard files (Optional Layer 4)
  if (answers?.preferences?.wantsDashboard) {
    files['memory/command-center.md'] = `<!-- verified: ${today} | scope: Command Center Dashboard Architecture | salience: 3.00 | hits: 0 -->
<!-- keywords: dashboard, command center, flask, react, tauri, ui, interface -->

# Command Center Dashboard

This is the Layer 4 Dashboard for the memory system.
It relies on JSON files generated by the Hourly Refresh task.

## Architecture
- Backend: Flask API serving JSON files from memory/
- Frontend: React / Tauri
- Data Flow: Scheduled task -> JSON files -> Flask -> React -> User Commands -> replies.json
`;
  }
...
function getSetupMd(answers) {
  ...
  const toolsText = Array.isArray(answers?.tools?.tools) ? answers.tools.tools.join(' ') : safeLower(answers?.tools?.tools);
  const integrations = Array.isArray(answers?.tools?.integrations) ? answers.tools.integrations : [];
  const hasSlack = toolsText.includes('slack') || integrations.some(int => safeLower(int).includes('slack'));
...
\`\`\`
Add Gmail, Google Calendar${hasSlack ? ', and Slack' : ''}.
...
function getHourlyRefreshPrompt(answers) {
  const name = answers?.identity?.name || 'User';
  const safeLower = (val) => String(val || '').toLowerCase();
  const toolsText = Array.isArray(answers?.tools?.tools) ? safeLower(answers.tools.tools.join(' ')) : safeLower(answers?.tools?.tools);
  const osText = Array.isArray(answers?.tools?.os) ? safeLower(answers.tools.os.join(' ')) : safeLower(answers?.tools?.os);
  const integrations = Array.isArray(answers?.tools?.integrations) ? answers.tools.integrations : [];
  const hasSlack = toolsText.includes('slack') || integrations.some(int => safeLower(int).includes('slack'));
  const hasApple = osText.includes('mac') || osText.includes('darwin') || answers?.tools?.integrations?.includes('apple');
  const wantsDashboard = answers?.preferences?.wantsDashboard || false;

  let steps1to5 = `**Steps 1-5**: Pull email (7d, max 15, filter spam), calendar (all calendars, 10d, merge+dedup), extract email action items to TASKS.md`;
  if (hasSlack) steps1to5 += `, pull Slack (to:me + configured channels, max 15)`;
  if (hasApple) steps1to5 += `, pull iMessages/Reminders via AppleScript if available`;
  steps1to5 += `.`;
...
**Step 0**: Read memory/assistant/thinking.md (FIRST), briefing.md, patterns.md, relationships.md, timeline.md${wantsDashboard ? ', and dashboard replies. Unread replies override assumptions.' : '.'}

${steps1to5}

**Step 6 THINK**: Before classifying anything, reason:
(a) What changed? (b) What patterns across sources? (c) What should user know unprompted? (d) Project assessments? (e) Relationship reads? (f) What would you advise today? Ground in evidence. Hold in memory.
${wantsDashboard ? `\n**Step 7 (dashboard)**: Classify into suggested events/tasks. Merge rules: read existing files FIRST, keep dismissed/accepted/rejected, dedup by (title,sender) AND email_id, cap 15 events + 20 tasks. Read suggestion_feedback.json (last 50) to calibrate.\n**Step 8 (dashboard)**: Score project activity across sources.` : ''}
...
${wantsDashboard ? `\n- Prune feedback to 50 entries, mark replies read.\n\n**Steps 10-12 (dashboard)**: Write header.json (greeting + tagline + 3 priority items), write data JSONs (MERGE projects, don't overwrite suggested/pending/replies), process queued commands (draft only, never auto-send).\n**Step 14**: Verify meta.json was written.` : ''}
...
function getQuickstartGuide(answers) {
  const name = answers?.identity?.name || 'User';
  const toolsText = (answers?.tools?.tools || '').toLowerCase();
  const hasSlack = toolsText.includes('slack') || answers?.tools?.integrations?.includes('slack');
...
Add Gmail, Google Calendar${hasSlack ? ', and Slack' : ''}. This takes about 2 minutes.
